...
给 COMP10002(C 语言算法课)学生做的 **macOS 本地 Electron 算法学习应用**:导入 C 代码 → 呈现为可视化积木结构 → 点击积木/代码互相高亮 → 受控地编辑积木反向修改代码 → 叠加确定性的程序分析(CFG、数据流、内存风险)与算法模式识别(线性搜索/二分/排序……,带置信度)→ 通过测试、执行轨迹、复杂度与正确性论证形成学习闭环 → AI 只负责解释和教学。
...
10. 核心转换 100% 确定性;AI 不参与解析、类型检查、代码生成、语义等价判定。
...
**D10 · AI 物理隔离,只读进场,M7 才接入。** 详见 §8;M8 的学习闭环可以消费 AI 解释,但不得把 AI 变成核心能力的硬依赖。
...
**M7 · AI 解释层。** Electron main 的 `panel:explain` handler 调本机 Ollama(qwen3.5:9b) `/api/chat`,preload 只暴露 `window.panelApi.explain`;prompt 注入选中积木原文 + M5/M6 确定性事实(AI 只复述与教学,不产生新断言);渐进提示与苏格拉底提问;`src/ai/` 物理隔离。验收:`check-ai-isolation.mjs` 静态遍历 `src/ai/` import 图,出现补丁引擎或任何写接口即 fail;mock Ollama 单测 + Electron IPC E2E;Ollama 离线时 AI 增强区优雅缺席、M2 的确定性解释面板与其余全功能不受影响。**不做**:AI 生成/修改代码、AI 自动贴标签。
...
**可以**:解释选中积木/代码(注入确定性分析事实后复述与扩展)、推测变量与函数的业务含义(标"推测")、对已识别模式做自然语言教学、渐进提示(先问后答)、根据编辑轨迹与报错教学、提议更高层积木合并(仅提议,确定性引擎验证后经用户确认)。

**不可以**:参与 C 解析、类型检查、生成或修改代码、单独裁定语义等价、在任何置信度下自动改程序、给算法贴标签(标签只能来自 §5 规则引擎)。

**接入**:默认本地 Ollama qwen3.5:9b(`/api/chat`,仅 Electron main 可访问),renderer 经 `window.panelApi.explain` 请求;离线优雅缺席。`src/ai/` 与 `src/core/` 物理隔离,import 图白名单由脚本强制。
